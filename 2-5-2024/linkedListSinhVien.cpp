...
void Insert(List &list) //Phương trình nhập phần tử vào linked list
{
    Node* temp = new Node();
...
int numberOfElement (List List) //Phương trình tìm số phần tử của 1 linked list
{
    int i = 1;
...
int studentIDIndexing(List List, int i)//Phương trình tìm số ID của phần tử trong linked list dựa vào index phần tử ấy
{
        Node* temp = List.first;
        int a = 0;
        while (a < i)
...
void swapNodesFromPos (List List, int a, int b)//Phương trình đổi chỗ 2 node trong 1 linked list
{
    int i = 0;
    int j = 0;
...
void Print(List list)//Phương trình in linked list cơ bản
{
    Node* temp = list.first;
    cout << "\nDanh sach sinh vien: \n";
    while (temp!= NULL)
...
void selectionSort(List List, int n)//Phương trình selection sort trong linked list
{
    for (int i = 0; i < n-1; i++)
    {
        int minPos = i;
